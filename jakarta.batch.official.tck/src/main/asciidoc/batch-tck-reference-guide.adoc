...
You will need to set one required property, *batch.impl.testng.path* prior to running the Jakarta Batch TCK. This property is defined in the `batch-tck.properties` file as follows:
...
An optional property with name *jvm.options* is provided to specify JVM arguments using the **TestNG** `<jvmarg line=""/>` function: This property value should list the desired JVM arguments, separated by spaces.
...
It may be possible to use the TestNG `build.xml` script's *compile* target, after setting the *src* property appropriately, but this is an older usage that we haven't focused on keeping updated.   More recently we have focused on the Maven approach.
